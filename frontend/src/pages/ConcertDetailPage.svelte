...
{:else if !concertData || error === "not-found"}
  <NotFoundPanel title="Konser tidak ditemukan." message="Konser ini mungkin sudah tidak tersedia atau URL-nya tidak tepat." />
{:else if error}
  <section class="shell empty-state" role="alert"><p>{error}</p><button class="text-button" type="button" onclick={() => load()}>Coba lagi</button></section>
{:else}

...
export function checkoutLines(concert: Concert, params: URLSearchParams): CheckoutLine[] {
  return concert.ticketTiers.flatMap((tier) => {
    ...
    return quantity > 0 && quantity <= limit ? [{ tier, quantity }] : [];
  });
}
...
export function isValidPhone(value: string) { return /^[0-9+][0-9 -]{8,16}$/.test(value); }
...
if (typeof document !== "undefined") {
  ...
  if (!concert) {
    ...
    if (!lines.length) {
      ...
      app.innerHTML = `<section class="checkout shell"><a class="back-link" href="/konser/${concert.id}?${location.search.slice(1)}">Kembali ke detail konser</a><div class="checkout-heading"><p class="checkout-kicker">Checkout aman</p><h1>Selesaikan pesananmu.</h1><p>${concert.artist} · ${concert.date} · ${concert.venue}</p></div><ol class="checkout-steps" aria-label="Tahap checkout"><li data-step="1" aria-current="step"><span>1</span><b>Data Diri</b></li><li data-step="2"><span>2</span><b>Metode Bayar</b></li><li data-step="3"><span>3</span><b>Konfirmasi</b></li></ol><div class="checkout-layout"><section class="checkout-panel" aria-live="polite"><form id="buyer-form" novalidate><div class="step-title"><p>Langkah 1 dari 3</p><h2>Data pemesan</h2><span>Informasi ini digunakan untuk mengirim e-ticket.</span></div><div class="field-grid"><label>Nama lengkap<input name="name" autocomplete="name" required /></label><label>Email<input name="email" type="email" autocomplete="email" required /><small data-error="email"></small></label><label>No. HP<input name="phone" inputmode="tel" autocomplete="tel" required /><small data-error="phone"></small></label><label>No. identitas<input name="identity" inputmode="numeric" required /><small data-error="identity"></small></label></div><div class="step-actions"><span></span><button class="button" type="submit">Lanjut ke pembayaran</button></div></form><form id="payment-form" hidden><div class="step-title"><p>Langkah 2 dari 3</p><h2>Pilih metode bayar</h2><span>Pilih salah satu metode untuk pesanan ini.</span></div><fieldset><legend class="sr-only">Metode pembayaran</legend><div class="payment-options"><label class="payment-card"><input type="radio" name="payment" value="BCA Virtual Account" /><span class="payment-logo bca">BCA</span><span><b>BCA Virtual Account</b><small>Verifikasi otomatis</small></span></label><label class="payment-card"><input type="radio" name="payment" value="Mandiri Virtual Account" /><span class="payment-logo mandiri">mandiri</span><span><b>Mandiri Virtual Account</b><small>Verifikasi otomatis</small></span></label><label class="payment-card"><input type="radio" name="payment" value="QRIS" /><span class="payment-logo qris">QRIS</span><span><b>QRIS</b><small>Scan dari aplikasi pembayaran</small></span></label><label class="payment-card"><input type="radio" name="payment" value="GoPay" /><span class="payment-logo gopay">GoPay</span><span><b>GoPay</b><small>Bayar dari aplikasi GoPay</small></span></label></div><small class="form-error" id="payment-error"></small></fieldset><div class="step-actions"><button class="text-button" type="button" data-back="1">Kembali</button><button class="button" type="submit">Tinjau pesanan</button></div></form><section id="confirmation" hidden><div class="step-title"><p>Langkah 3 dari 3</p><h2>Periksa pesanan</h2><span>Pastikan data dan metode pembayaran sudah sesuai.</span></div><div class="confirmation-block"><div><b data-buyer-name></b><span data-buyer-contact></span></div><button class="text-button" type="button" data-edit="1">Ubah</button></div><div class="confirmation-block"><div><b data-payment></b><span>Metode pembayaran terpilih</span></div><button class="text-button" type="button" data-edit="2">Ubah</button></div><div class="step-actions"><button class="text-button" type="button" data-back="2">Kembali</button><button class="button" type="button" id="place-order">Buat pesanan</button></div></section><section id="success" class="checkout-success" hidden><p class="checkout-kicker">Pesanan dibuat</p><h2>Sampai jumpa di konser.</h2><p>E-ticket untuk <b>${ticketText}</b> akan dikirim ke <b data-success-email></b>.</p><div class="booking-code"><span>Kode booking</span><strong id="booking-code"></strong></div><p class="success-note">Ini adalah simulasi frontend. Tidak ada pembayaran atau reservasi stok yang diproses.</p><a class="button" href="/konser">Jelajahi konser</a></section></section><aside class="order-summary"><div class="summary-concert"><img src="${concert.image}" alt="Poster contoh konser ${concert.artist}" width="90" height="112" /><div><p>${concert.status}</p><h2>${concert.artist}</h2><span>${concert.date}</span><span>${concert.venue}, ${concert.city}</span></div></div><div class="summary-lines">${lines.map(({ tier, quantity }) => `<div><span>${quantity}x ${tier.name}</span><b>${formatRupiah.format(tier.price * quantity)}</b></div>`).join("")}</div><form class="voucher-form" id="voucher-form"><label for="voucher">Kode promo</label><div><input id="voucher" name="voucher" placeholder="Contoh: HEMAT10" /><button type="submit">Pakai</button></div><small id="voucher-message"></small></form><div class="summary-total"><div><span>Subtotal</span><b>${formatRupiah.format(subtotal)}</b></div><div><span>Biaya admin</span><b>${formatRupiah.format(ADMIN_FEE)}</b></div><div id="discount-row" hidden><span>Diskon</span><b id="discount-value"></b></div><div class="grand-total"><span>Total</span><strong id="grand-total">${formatRupiah.format(total())}</strong></div></div></aside></div></section>`;
      ...
      function validateBuyer() { return (["name", "email", "phone", "identity"] as const).every(validateField); }
      ...
      paymentForm.addEventListener("submit", (event) => { event.preventDefault(); payment = (paymentForm.elements.namedItem("payment") as RadioNodeList).value; const error = document.querySelector<HTMLElement>("#payment-error")!; error.textContent = payment ? "" : "Pilih metode pembayaran terlebih dahulu."; if (!payment) return; document.querySelector<HTMLElement>("[data-buyer-name]")!.textContent = buyer.name; document.querySelector<HTMLElement>("[data-buyer-contact]")!.textContent = `${buyer.email} · ${buyer.phone}`; document.querySelector<HTMLElement>("[data-payment]")!.textContent = payment; setStep(3); });

<script lang="ts">
  ...
  let step = $state(1); let payment = $state(""); let voucherInput = $state(""); let voucher = $state(""); let completed = $state(false); let ticketId = $state(""); let storageFailed = $state(false);
  ...
  async function completeOrder() {
    if (!concert) return;
    completed = true; ticketId = crypto.randomUUID();
    const reference = `TO-${crypto.randomUUID().replaceAll("-", "").slice(0, 10).toUpperCase()}`;
...
{:else}
  <section class="checkout shell"><a class="back-link" href={`/konser/${concert.id}?${location.search.slice(1)}`}>Kembali ke detail konser</a><div class="checkout-heading"><p class="checkout-kicker">Checkout aman</p><h1>Selesaikan pesananmu.</h1><p>{concert.artist} · {concert.date} · {concert.venue}</p></div><ol class="checkout-steps" aria-label="Tahap checkout">{#each ["Data Diri", "Metode Bayar", "Konfirmasi"] as label, index}<li class:is-complete={index + 1 < step || completed} aria-current={index + 1 === step && !completed ? "step" : undefined}><span>{index + 1}</span><b>{label}</b></li>{/each}</ol>
    <div class="checkout-layout"><section class="checkout-panel">
      {#if completed}<section class="checkout-success"><p class="checkout-kicker">Pesanan berhasil</p><h2 id="success-title" tabindex="-1">E-ticket sudah dibuat.</h2><p>Konfirmasi demo telah disiapkan untuk {buyer.email}.</p>{#if storageFailed}<p class="success-note">E-ticket demo tidak dapat disimpan di browser ini.</p>{:else}<a class="button" href={`/tiket/${ticketId}`}>Lihat e-ticket</a>{/if}<a class="button button-secondary" href="/konser">Cari konser lain</a></section>
      ...
      {:else if step === 2}<form id="payment-form" onsubmit={submitPayment}><div class="step-title"><p>Langkah 2 dari 3</p><h2 id="payment-title" tabindex="-1">Pilih metode bayar.</h2><span>Pembayaran di halaman ini hanya simulasi.</span></div><fieldset class:has-error={Boolean(paymentError)} class="payment-options"><legend class="sr-only">Metode pembayaran</legend>{#each ["QRIS", "Virtual Account", "GoPay"] as method}<label class:gopay={method === "GoPay"}><input bind:group={payment} type="radio" name="payment" value={method} aria-describedby="payment-error" /><span>{method}</span></label>{/each}</fieldset><p id="payment-error" class="field-error" aria-live="polite">{paymentError}</p><div class="step-actions"><button class="text-button" type="button" onclick={() => goToStep(1)}>Kembali</button><button class="button" type="submit">Tinjau pesanan</button></div></form>
      {:else}<section id="confirmation"><div class="step-title"><p>Langkah 3 dari 3</p><h2 id="confirmation-title" tabindex="-1">Periksa sebelum memesan.</h2><span>Pastikan data dan pesananmu sudah benar.</span></div><div class="confirmation-grid"><div><span>Pemesan</span><b>{buyer.name}</b><p>{buyer.email} · {buyer.phone}</p><button class="text-button" type="button" onclick={() => goToStep(1)}>Ubah data</button></div><div><span>Pembayaran</span><b>{payment}</b><button class="text-button" type="button" onclick={() => goToStep(2)}>Ubah metode</button></div></div><div class="step-actions"><button class="text-button" type="button" onclick={() => goToStep(2)}>Kembali</button><button class="button" type="button" onclick={completeOrder}>Buat pesanan</button></div></section>{/if}
